...
export class PaymentVerificationService {
 ...
 private async bound(store:DraftStore,context:AuthContext,id:string){
  ...
  if(!quote||!payment||row._count.payments!==1||quote.listingId!==id||payment.listingId!==id||payment.userId!==context.user.id
 ...
 async verify(context:AuthContext,id:string){
  ...
  const before=await this.repository.withProvider(context.user.id,store=>this.bound(store,context,id));
  ...
  return this.repository.withProvider(context.user.id,async store=>{
   const {row,payment}=await this.bound(store,context,id);
   ...
   const settled=await store.settlePayment(payment.id,verified.status==='success'?'SUCCEEDED':'FAILED');
