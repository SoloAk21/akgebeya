  ...
  try {
    ...
    async function snapshot() {
      ...
      for (const table of tables) {
        ...
        const expression = Prisma.raw(table.schemaname === 'akgebeya' && table.tablename === 'users'
          ? "to_jsonb(t) - 'telegramId'" : 'to_jsonb(t)');
    ...
    const nonNull = await db.$queryRaw<{ count: number }[]>
      `SELECT count(*)::int AS count FROM akgebeya.users WHERE "telegramId" IS NOT NULL`;
